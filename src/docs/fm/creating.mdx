...
To show you how functional modules work under the hood we will go with the second approach, in your text editor go under `/modules` and create a new folder call it `todo` you can call it whatever seems fit but we'll be making a tasks module.
...
Under tasks create another folder called `bootstraps` and under that create a file called `main.server.bootsrap.js`.
...
we will create two controllers for the `list` and `create` tasks
  ...
  routes: [
    {
      ...
      methods: {
        get: {
          iam: "modules:tasks:main:list",
          title: "List tasks",
          groups: [],
          parents: ["modules:tasks", "modules:tasks:main"],
          description: "List available tasks",
          middlewares: [crtls.list],
        },
        {
        post: {
          iam: "modules:tasks:main:create",
...
And now our next step is to create a model for our tasks
...
Now let's use our newly created model in the controller, on the top of the file require in the model from [mongoose](https://mongoosejs.com/) and do the logic for listing our tasks and creating a task
