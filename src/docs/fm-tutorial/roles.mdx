...
> As a convention iams must be named as the following: `"<containing-folder>:<child-folder>:file"` and the forth component of the iam you're free to name it what you want, we name it according to the name of the controller.
...
And If everything goes well you should get a response with your newly user created like the following
...
Hmmm Bummer, it's says that the user isn't authorized... Well, it makes sense because we haven't setup our iam for the user yet, now let's go ahead and asign to the use the iam that allows him to get the list of the todos, and it is as simple as putting the `"modules:todo:main:list"` iam in the user iam array.
